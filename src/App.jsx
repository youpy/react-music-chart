...
export default class App extends Component {
  ...
  onChangeArtistFilter(e) {
    this.setState(Object.assign({}, this.state, { artistFilter: e.target.value.toLowerCase() }));
  }

  onChangeTitleFilter(e) {
    this.setState(Object.assign({}, this.state, { titleFilter: e.target.value.toLowerCase() }));
  }

  onChangeLabelFilter(e) {
    this.setState(Object.assign({}, this.state, { labelFilter: e.target.value.toLowerCase() }));
  }

  onChangeGenreFilter(e) {
    this.setState(Object.assign({}, this.state, { genreFilter: e.target.value.toLowerCase() }));
  }

  onChangeChartByFilter(e) {
    this.setState(Object.assign({}, this.state, { chartByFilter: e.target.value.toLowerCase() }));
  }
              ...
              Object.keys(item.chart_by).map((who) =>
                <li
                    ...
                    style={{
                        ...
                        background: who.toLowerCase() === this.state.chartByFilter ? '#fcc' : null
                      }}>
                  <a href={`#chart_by=${encodeURIComponent(who)}`}>{who}</a>: {item.chart_by[who]}
                </li>)
            }
          </ul>
        </div>
  ...
  render() {
    const items = this.props.chart.filter((item) => {
      if (this.state.artistFilter !== '') {
        return item.artist.toLowerCase().indexOf(this.state.artistFilter) !== -1;
      } else {
        return true;
      }
    }).filter((item) => {
      ...
      if (this.state.chartByFilter !== '') {
        return Object.keys(item.chart_by).join(' ').toLowerCase().indexOf(this.state.chartByFilter) !== -1;
      } else {
        return true;
      }
    });
    ...
    return (
      <div>
        <h1><a href="./">Boomkat Charts 2017: Merged</a></h1>
        <div className="row">
